...
router.post("/many", async (req, res) => {
  ...
  try {
    await db.sequelize.models.Game.bulkCreate(req.body, {
      validate: true,
    }).then(games => {
      console.log(
        "New games created successfully, query the database to see them",
      );
      // console.log(game.toJSON());
      res.status(200).send({
        test: "Successfully created new games.",
        newGames: games.map(game => game.toJSON()),
      });
    });
  } catch (err) {
    console.error("Error creating new games:", err);
    res
      .status(500)
      .send({ error: "An error occurred while creating the games." });
    return;
  }
});
